...
static void bcm6348_pci_setup(struct bcm6348_pci *priv)
{
	...
	__raw_writel(~(priv->mem_size - 1), priv->pci + MPI_SP0_RANGE_REG);
...
static int bcm6348_pci_probe(struct platform_device *pdev)
{
	...
	struct bcm6348_pci *priv = &bcm6348_pci;
	struct resource *res;
	const __be32 *mem_addr;
	u64 mem_size;
	unsigned int mem_flags;

	printk("%s\n", __func__);

	mem_np = of_find_node_by_name(NULL, "memory");
	mem_addr = of_get_address(mem_np, 0, &mem_size, &mem_flags);
	of_node_put(mem_np);
	if (!mem_addr)
		return -ENODEV;

	priv->mem_size = (u32) mem_size;
